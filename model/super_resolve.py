...
flags.DEFINE_string('checkpoint_dir', 'results/64-32-3_9-1-5_100', 'Checkpoint directory.')
flags.DEFINE_string('image_file', 'hahah2.png', 'Sample image file.')
...
# psnr = []
for j in range(1):
    hr = cv2.resize(hr, (0, 0), fx=4., fy=4., interpolation=cv2.INTER_CUBIC)
    feed_dict = {x: hr[np.newaxis], is_training: False}
    hr = sess.run(model.prediction, feed_dict=feed_dict)[0]

print(hr.shape)
#     psnr.append(sess.run([bic_psnr], feed_dict={is_training: False, x_placeholder: xtest,
#                                                                  y_placeholder: ytest}))
# # print(type(img[0, 0, 0]))

ori = cv2.imread('hahah2ori.png')
print(ori.shape)
